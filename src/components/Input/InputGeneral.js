...
import { voiceMemoIcon } from './util/voiceIcon';
// import VoiceMemo from '../util/VoiceMemo';
...
const VoiceMemoButton = styled.div`
  height: 28px;
  width: 28px;
  border-radius: 14px;
  background: pink;
...
class InputGeneral extends Component {
  ...
  componentDidUpdate() {
    const { showVoiceMemo } = this.state;
    const { value } = this.props;
    if (!value && !showVoiceMemo) {
    ...
    return (
      <React.Fragment>
        {showVoiceMemo ? (
					null
          // <VoiceMemo
          //   closeVoiceMemo={this.closeVoiceMemo}
